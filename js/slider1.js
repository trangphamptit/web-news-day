...
$(document).ready(function () {
    ...
    function getPostsFromServer() {
        $.ajax({
            url: 'https://api.myjson.com/bins/' + detailCodeParam,
            method: "GET",
            data: { a: "B" },
            success: onSuccess,
            error: function (error) {
    ...
    function getItemHtml(title, time, image, detail,author) {
        var itemHtml = '<div class="title">'+ title+ ' </div> <div class="time"><i class="fa fa-clock-o" aria-hidden="true"></i>'
        +time+'  </div>  <img class="mainContent-image" src="'+ image+'" alt="">'+' <div class="detail">'+ detail+' </div><div class="author"><i>'+ author+'</i></div>'
    ...
    function onSuccess(posts, a, b) {
        ...
        for (var i = 0; i < list.length; i++) {
            html += getItemHtml(list[i].title, list[i].time, list[i].image, list[i].detail,list[i].author);
        }
